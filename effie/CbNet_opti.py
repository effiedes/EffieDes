...
class Features(nn.Module):
    ...
    def forward(self, crd, max_neigh=128):
...
class CbNet_v2(nn.Module):
    ...
    def forward(self, crd, thresh=15, max_neigh = 128, missing = None, chain_idx = None, tm = None):
    
        """
        Through the neural net
        Input: coordinates (tensor n*4*3), whose features will be calculated
               thresh: distance cut-off in Angstrum to decide which pairs to keep (int or Nones) 
        ...
        if self.unary:
            if self.transmembrane:
                unary_costs = self.resnet_unary(torch.cat((ft, tm.reshape(-1, 1)), dim = -1))
            else:
                unary_costs = self.resnet_unary(ft)
        ...
        inv_idx_pairs[torch.arange(nb_var, device = crd.device)[:,None],idx_pairs] = torch.arange(
        ...
        out = out.reshape(nb_var,nb_neigh,out.shape[-1])
        out = out[idx_unique[0],idx_unique_pairs.type(torch.LongTensor)]
        out = out.reshape(-1, out.shape[-1])
        out = self.resnet(out) #throught tne neural net

        out = out[inverse_indices]
        out[trans] = out[trans].reshape(-1,self.nb_aa, self.nb_aa).transpose(1,2).reshape(
            -1,self.nb_aa*self.nb_aa)
        out_complete = torch.zeros((nb_var*nb_neigh,self.nb_aa* self.nb_aa),device = crd.device)
        out_complete[to_keep] = out
        out_complete = out_complete.reshape(nb_var,nb_neigh,-1)
